...
# Mobile -> Backend -> Admin shared working prototype
...
app = FastAPI(title="WorkforceFit AI API", version="2.0.0")
...
for lang, trade, text, kws in DEFAULT_QUESTIONS:
    ...
    questions_db[qid] = {"id": qid, "language": lang, "trade": trade, "text": text, "expectedKeywords": kws, "maxScore": 20, "active": True}
...
def score_answer(answer: str, keywords: List[str]) -> dict:
    ...
    keyword_hits = sum(1 for k in keywords if normalize(k) and normalize(k) in txt)
    keyword_score = keyword_hits / max(len(keywords), 1)
    has_specifics = 1.0 if any(ch.isdigit() for ch in txt) or word_count >= 18 else 0.65
    relevance = max(0.45, 0.55 * keyword_score + 0.45 * length_score)
    clarity = max(0.50, min(1.0, 0.35 + length_score * 0.55 + (0.10 if "." in answer or "," in answer else 0)))
    confidence = max(0.50, min(1.0, 0.40 + has_specifics * 0.30 + length_score * 0.30))
    overall = round((relevance * 0.45 + clarity * 0.25 + confidence * 0.30), 3)
    return {
...
@app.get("/")
def root():
    return {"status": "ok", "service": "WorkforceFit AI", "docs": "/docs"}
...
async def register_candidate(data: CandidateRegister):
    for c in candidates_db.values():
        if c["phone"] == data.phone:
            raise HTTPException(400, "Candidate with this phone already registered")
    ref_id = data.refId or ref_id_for(data.district)
    candidates_db[ref_id] = {
        **data.dict(), "refId": ref_id,
        "status": "REGISTERED",
    ...
    return {"refId": ref_id, "candidate": candidates_db[ref_id], "message": "Registration successful"}

@app.get("/api/questions")
def get_questions(language: str = "en", trade: str = "General"):
    exact = [q for q in questions_db.values() if q["active"] and q["language"] == language and (q["trade"] == trade or q["trade"] == "General")]
    if not exact:
        exact = [q for q in questions_db.values() if q["active"] and q["language"] == "en"]
    return exact[:5]

@app.post("/api/admin/questions")
def create_question(q: QuestionCreate):
    qid = f"Q-{uuid.uuid4().hex[:8].upper()}"
    questions_db[qid] = {"id": qid, **q.dict(), "active": True}
...
async def verify_liveness(refId: str, video: UploadFile = File(...)):
    if refId not in candidates_db:
        raise HTTPException(404, "Candidate not found")
    face_hash = hashlib.sha256(f"{refId}-face".encode()).hexdigest()
...
async def evaluate_response(data: EvaluateRequest):
    ...
    result = score_answer(data.transcript, q.get("expectedKeywords", []))
    record = {"questionId": data.questionId, "question": q["text"], "answer": data.transcript, **result}
...
async def classify_candidate(ref_id: str):
    ...
    avg = sum(s["overall"] for s in question_scores) / len(question_scores)
    liveness = candidates_db[ref_id].get("livenessScore", 0.5)
    duplicate = candidates_db[ref_id].get("duplicateFlag", False)
    ...
    candidates_db[ref_id].update({"fitment": fitment, "score": score_int, "status": "CLASSIFIED"})
    return {"fitment": fitment, "score": score_int, "answers": answers_db.get(ref_id, [])}
...
async def admin_get_candidates(district: Optional[str] = None, fitment: Optional[str] = None):
    ...
    if fitment and fitment != "All":
        result = [c for c in result if c.get("fitment") == fitment]
    return result

@app.get("/api/admin/stats")
async def admin_stats():
    classified = [c for c in candidates_db.values() if c.get("fitment")]
    by_cat = Counter(c["fitment"] for c in classified)
    by_dist = Counter(c["district"] for c in candidates_db.values())
    return {
        "total": len(candidates_db),
        "registered": len(candidates_db),
        "classified": len(classified),
        "flagged": len([c for c in candidates_db.values() if c.get("duplicateFlag") or c.get("fraudScore", 0) > 0.7]),
        "byCategory": dict(by_cat),
        "byDistrict": [{"district": k, "count": v} for k, v in by_dist.most_common(10)],
        "distribution": [{"name": k, "value": v} for k, v in by_cat.items()],
    }

@app.post("/api/admin/candidates/{ref_id}/action")
async def admin_action(ref_id: str, data: AdminAction):
    if ref_id not in candidates_db:
        raise HTTPException(404, "Candidate not found")
    candidates_db[ref_id]["status"] = data.action
    return {"refId": ref_id, "status": data.action}
